...
class Logger():
    csv_header = \
    "timeDate,avrgCo2,pState0,pState1,pState2,pState3,pState4"
    def __init__(self):
        pass
    ...
    def log(cls,prefix: str, message: str, file="main_log.txt"):
        ...
        my_path = os.getcwd().split("airTrackerEval", 1)[0]
        my_path = os.path.join(my_path, "airTrackerEval", "log", file)
        
        #write to file
        if not os.path.exists(my_path):
            with open(my_path, "w", encoding=("UTF-8")) as f:
                f.write("Log-File for Air-airTrackerEval. Created:"\
    ...
    def log_csv(cls, message: tuple, file="data"):
        ...
        #create correct path to file
        my_path = os.getcwd().split("airTrackerEval", 1)[0]
        my_path = os.path.join(my_path, "airTrackerEval", f"{file}.csv")
